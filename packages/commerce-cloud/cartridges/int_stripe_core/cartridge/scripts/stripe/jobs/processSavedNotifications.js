...
function getStripeCustomerIdFromOrder(order) {
    ...
    if (
        orderCustomer &&
        orderCustomer.profile &&
        'stripeCustomerID' in orderCustomer.profile.custom &&
        orderCustomer.profile.custom.stripeCustomerID
    ) {
...
function createCharge(stripeNotificationObject, order, stripePaymentInstrument) {
    ...
    var stripeCustomerId =
        stripePaymentInstrument.custom.stripeCustomerID || getStripeCustomerIdFromOrder(order)
    ...
    try {
        ...
        Transaction.wrap(function () {
            ...
            if (charge.balance_transaction && stripePaymentInstrument.paymentTransaction) {
                stripePaymentInstrument.paymentTransaction.transactionID =
                    charge.balance_transaction // eslint-disable-line
        ...
        stripeLogger.info(
            'Charge was successfull for order {0}, CO event id {1}, source {2}',
            order.orderNo,
        ...
        stripeLogger.info(
            'Unsuccessful charge for order {0}, CO event id {1}, source {2} message {3}',
            order.orderNo,
            stripeNotificationObject.custom.stripeEventId,
            stripeNotificationObject.custom.stripeSourceId,
            e.message
...
function failOrder(stripeNotificationObject, order) {
    ...
    var failedDetailsForOrder = {
        failure_code: chargeJSON.data.object.failure_code
            ? chargeJSON.data.object.failure_code
            : '',
        failure_message: chargeJSON.data.object.failure_message
            ? chargeJSON.data.object.failure_message
            : '',
        fraud_details: chargeJSON.data.object.fraud_details
            ? chargeJSON.data.object.fraud_details
            : '',
    ...
    Transaction.wrap(function () {
        order.addNote(
            'Stripe Processing Job Note(failed details)',
            JSON.stringify(failedDetailsForOrder)
        )
    ...
    // TODO: We need to decide if we want to send out an email
    // var statusMail = sendMail(
    //     order,
    //     'stripe/mail/orderfailed',
    //     Resource.msg('order.orderfailed-email.001', 'stripe', null)
    // )

    stripeLogger.info(
        '\nSuccessfully proccesed CO with event id: {0}, source id: {1}, updated SFCC order status to "FAILED". Set up CO processingStatus to {2}, email send - {3}',
        stripeNotificationObject.custom.stripeEventId,
        stripeNotificationObject.custom.stripeSourceId,
        'FAIL_OR_CANCEL',
        // statusMail.status === Status.OK ? 'true' : 'false'
        'false'
    )
}
...
function placeOrder(stripeNotificationObject, order, stripePaymentInstrument) {
    ...
    Transaction.wrap(function () {
        ...
        */
        ...
        if (placeOrderStatus === Status.ERROR) {
            ...
            stripeLogger.info(
                'An error occured durring place order: {0}, error message: {1}',
                order.orderNo,
                placeOrderStatus.message
            )
        }
    ...
    if (isAPMOrder) {
        ...
        // }
        stripeLogger.info(
            'Successfully proccesed CO with event id: {0}, source id: {1} , updated SFCC order status to "EXPORT_STATUS_READY". Set up CO processingStatus to {2}, email send - {3}',
            stripeNotificationObject.custom.stripeEventId,
            stripeNotificationObject.custom.stripeSourceId,
            'PROCESSED',
            // statusMail.status === Status.OK ? 'true' : 'false'
        ...
        stripeLogger.info(
            'Successfully proccesed CO with event id: {0}, source id: {1} , updated SFCC order status to "EXPORT_STATUS_READY". Set up CO processingStatus to {2}, email Not sent',
            stripeNotificationObject.custom.stripeEventId,
            stripeNotificationObject.custom.stripeSourceId,
            'PROCESSED'
        )
...
function processChargeRefunded(stripeNotificationObject, order, stripePaymentInstrument) {
    ...
    Transaction.wrap(function () {
        ...
        const noteMessage = Resource.msgf(
            'order.amountrefundedmsg',
            'stripe',
            null,
            moneyRefunded.toString(),
            order.getOrderNo()
        )
...
function placeOrderAfterReview(stripeNotificationObject, order) {
    ...
    //         Resource.msg('order.orderconfirmation-email.001', 'order', null)
    //     )
    // }
...
function failOrderAfterReview(stripeNotificationObject, order) {
    Transaction.wrap(function () {
        order.addNote(
            'Stripe Processing Job Note(failed details)',
            stripeNotificationObject.custom.stripeWebhookData
        )
        var failStatus = OrderMgr.failOrder(order, true)
        if (failStatus.status === Status.ERROR) {
            stripeLogger.info('\n' + failStatus.message)
...
function processNotificationObject(stripeNotificationObject) {
    ...
    if (!coStripeSourceId && !coStripePaymentIntentId) {
        stripeLogger.info(
            '\nBoth Payment Intent Id and Source id are empty, event id: {0}',
            stripeEventId
        )
        return
    }
...
exports.execute = function () {
    ...
    try {
        stripeObjectsIter = CustomObjectMgr.queryCustomObjects(
            NOTIFICATIONS_CUSTOM_OBJECT_TYPE,
            queryString,
            null
        )
