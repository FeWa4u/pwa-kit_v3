...
function beforePaymentAuthorization() {
    ...
    const stripeAccountType =
        dw.system.Site.getCurrent().getCustomPreferenceValue('stripeAccountType')
    ...
    try {
        ...
        if (basket) {
            ...
            if (
                stripePaymentInstrument &&
                stripePaymentInstrument.paymentMethod === 'CREDIT_CARD'
            ) {
                paymentIntentId = stripePaymentInstrument.paymentTransaction
                    ? stripePaymentInstrument.paymentTransaction.getTransactionID()
                    : null
                if (paymentIntentId) {
                    paymentIntent = checkoutHelper.confirmPaymentIntent(
                        paymentIntentId,
                        stripePaymentInstrument
                    )
                } else {
                    ...
                    Transaction.wrap(function () {
                        stripePaymentInstrument.paymentTransaction.setTransactionID(
                            paymentIntent.id
                        )
                        stripePaymentInstrument.paymentTransaction.setType(
                            stripeChargeCapture
                        ...
                        if (!empty(stripeAccountId)) {
                            stripePaymentInstrument.paymentTransaction.custom.stripeAccountId =
                                stripeAccountId
                        }

                        if (
                            !empty(stripeAccountType) &&
                            'value' in stripeAccountType &&
                            !empty(stripeAccountType.value)
                        ) {
                            stripePaymentInstrument.paymentTransaction.custom.stripeAccountType =
                                stripeAccountType.value
                        }
                    })
                }
                ...
                Transaction.wrap(function () {
                    ...
                    if (
                        ...
                        basket.custom.stripeRiskLevel =
                            paymentIntent.charges.data[0].outcome.risk_level
                        basket.custom.stripeRiskScore =
                            paymentIntent.charges.data[0].outcome.risk_score
                    }
                })
                ...
                Transaction.wrap(function () {
                    ...
                    if (
                        stripePaymentInstrument &&
                        stripePaymentInstrument.paymentTransaction &&
                        stripePaymentInstrument.custom.stripeSourceID
                    ) {
                        stripePaymentInstrument.paymentTransaction.setTransactionID(
                            stripePaymentInstrument.custom.stripeSourceID
                        )
            ...
            if (
                ...
                Transaction.wrap(function () {
                    ...
                    if (
                        ...
                        order.custom.stripeRiskLevel =
                            paymentIntent.charges.data[0].outcome.risk_level
                        order.custom.stripeRiskScore =
                            paymentIntent.charges.data[0].outcome.risk_score
                    }
                })
...
function handleAPM(sfra) {
    ...
    const paymentIntentClientSecret = paramsMap.payment_intent_client_secret
        ? paramsMap.payment_intent_client_secret.stringValue
        : null
    ...
    try {
        ...
        if (!empty(paymentIntentId)) {
            ...
            if (
                ...
                const paymentMethod = stripeService.paymentMethods.retrieve(
                    paymentIntent.payment_method
                )
                if (paymentMethod && paymentMethod.link && paymentMethod.link.persistent_token) {
                    var stripeCookie = new dw.web.Cookie(
...
function beforePaymentSubmit(type, params) {
    ...
    try {
        const basketCurrencyCode = dw.util.Currency.getCurrency(basketCurrencyCode)
        ...
        if (type === 'paymentelement') {
            ...
            if (
                request.httpCookies['stripe.link.persistent_token'] &&
                request.httpCookies['stripe.link.persistent_token'].value
            ) {
        ...
        if (customer.authenticated && customer.profile && customer.profile.email) {
            ...
            if (
                type === 'paymentelement' &&
                stripeHelper.isStripePaymentElementsSavePaymentsEnabled()
            ) {
